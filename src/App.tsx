      ...
      <Layout>
        <Content className="app-content">
          <Routes>
            ...
            <Route path="/text-rule" element={<KnowledgeRulePage title="文本规则配置" columns={textRuleConfigData.columns} data={textRuleConfigData.rows} uniqueField="country" uniqueLabel="国家" requiredFields={['country', 'locale', 'requirement', 'currency', 'unitsMeasures', 'dateTimeFormat', 'localProductTerms', 'textStyle']} fieldOptions={{ country: ['美国', '加拿大', '英国', '意大利', '西班牙', '法国', '德国', '奥地利', '澳大利亚', '日本', '韩国', '泰国'], locale: ['英语（en-US）', '英语（en-CA）', '英语（en-GB）', '意大利语（it-IT）', '西班牙语（es-ES）', '法语（fr-FR）', '德语（de-DE）', '德语（de-AT）', '英语（en-AU）', '日语（ja-JP）', '韩语（ko-KR）', '泰语（th-TH）'] }} fieldDependencies={{ country: { targetField: 'locale', mapping: { '美国': '英语（en-US）', '加拿大': '英语（en-CA）', '英国': '英语（en-GB）', '意大利': '意大利语（it-IT）', '西班牙': '西班牙语（es-ES）', '法国': '法语（fr-FR）', '德国': '德语（de-DE）', '奥地利': '德语（de-AT）', '澳大利亚': '英语（en-AU）', '日本': '日语（ja-JP）', '韩国': '韩语（ko-KR）', '泰国': '泰语（th-TH）' } } }} filterFields={['country', 'locale', 'updateTime']} />} />
            <Route path="/image-rule" element={<KnowledgeRulePage title="图片规则配置" columns={imageRuleConfigData.columns} data={imageRuleConfigData.rows} uniqueField="moduleName" uniqueLabel="模块名称" requiredFields={['moduleName', 'mobileSize', 'appSize', 'allowedError']} fieldOptions={{ moduleName: ['轮播图', '通屏图文', '照片墙', '视频直播', '直播回放', '主推四款', '热卖分类', '图文展示', '热销款类', '网红背书', '九模块', '信用背书', '视频模块（大）', '视频模块（小）', '图片滑动', '一图片+一商品', '一图片+两商品', '一图片+三商品', '两行商品+一图片', '三图-上图下文', '两/三图-图文一体', '左图右文', '一图上下文', '一行图片（换行）', '轮播图+两图片', '单页四图', '背景图+一行品类', '品类页Banner', '品质保证', '邮件订阅', '预约有礼', '游戏入口'] }} filterFields={['moduleName', 'updateTime']} />} />
            <Route path="/page-rule" element={<KnowledgeRulePage title="页面规则配置" columns={pageRuleConfigData.columns} data={pageRuleConfigData.rows} uniqueField="ruleType" uniqueLabel="规则类型" requiredFields={['ruleType', 'criteria']} fieldOptions={{ ruleType: ['元素重叠', '容器溢出', '异常截断', '模块侵占'] }} filterFields={['ruleType', 'updateTime']} />} />
          </Routes>
        </Content>
      </Layout>
    </Layout>
